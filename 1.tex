...
附件 1 是离散时间序列，需要构造连续的环境边界函数$T_a(t)$、$C_a(t)$；说明选择分段线性插值的理由。药材近似为细长圆柱，长径比 $L/(2R)=6.25$，材料与边界在周向均匀；题目所列 0--2 cm 位置解释为中截面上到圆柱轴线的径向距离，因此采用一维轴对称径向模型。内部以热传导为主，表面与热风发生对流换热，构成非稳态热传导的第三类边值问题。内部水分迁移服从 Fick 扩散，且 $D$ 随 $C$ 非线性变化表面与环境通过对流传质交换水分。圆柱坐标在 $r=0$ 存在形式奇点，水分方程为非线性偏微分方程，环境边界又随时间变化，因此采用守恒型有限体积/有限元离散。COMSOL 负责有限元仿真，独立代码采用不同离散实现；以网格收敛性、物理单调性和两种求解结果的误差指标构成证据链。
