...
药材近似为半径 $R=0.02\ \mathrm{m}$、长度 $L=0.25\ \mathrm{m}$ 的
均匀圆柱体，其长径比为
\begin{equation}
\frac{L}{2R}=6.25.
\end{equation}
题目要求的到药材中心距离 $0\sim 2\ \mathrm{cm}$，解释为圆柱中截面上
到中心轴线的径向距离。
...
$L/2=12.5\ \mathrm{cm}$，在 $1800\ \mathrm{s}$ 的预热平衡阶段，
两个端面的扰动难以传播至圆柱中截面。因此忽略轴向和周向梯度，
仅保留径向变化，将药材中截面的传热传质过程简化为
\begin{equation}
0\le r\le R
\end{equation}
上的一维轴对称径向问题。
...
附件1给出了离散时刻的烘房温度和水分浓度。为获得数值计算所需的
连续边界函数，对相邻数据点进行分段线性插值。对于
$t_i\le t\le t_{i+1}$，定义
\begin{equation}
T_a(t)
=
T_{a,i}
...
线性插值不会在相邻测量点之间引入额外极值，可以避免高阶样条插值
可能产生的非物理过冲。COMSOL仿真与独立数值程序均采用上述边界函数，
以保证两种求解方法的输入条件一致。

\subsubsection{药材内部温度场模型}

根据能量守恒定律，单位体积药材的能量积累等于净导热流入。
由Fourier热传导定律
\begin{equation}
...
问题一中 $\rho$、$c_p$ 和 $k$ 均为常数，因此式
\eqref{eq:heat-equation}也可写为
\begin{equation}
...
\subsubsection{药材内部水分场模型}

药材内部水分迁移采用有效扩散理论描述。根据Fick扩散定律，
径向水分通量为
\begin{equation}
...
\end{equation}
由于 $D$ 随未知量 $C$ 变化，式\eqref{eq:moisture-equation}
属于变系数非线性抛物型偏微分方程。随着水分浓度降低，
$D(C)$ 逐渐减小，药材内部水分迁移阻力随之增大。
